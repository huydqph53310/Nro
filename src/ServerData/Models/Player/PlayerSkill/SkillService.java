                ...
                if (!player.playerSkill.prepareQCKK) {
                    ...
                    if (plTarget != null) {
                        ...
                        for (Mob mob : player.zone.mobs) {
                            if (!mob.isDie()&& Util.getDistance(plTarget, mob) <= SkillUtil.getRangeQCKK(player.playerSkill.skillSelect.point, player.zone.map.mapWidth)) {
                                mobs.add(mob);
                            }
                        }
                    }
                        ...
                        for (Mob mob : player.zone.mobs) {
                            if (!mob.equals(mobTarget) && !mob.isDie()
                                    && Util.getDistance(mob, mobTarget) <= SkillUtil
                                            .getRangeQCKK(player.playerSkill.skillSelect.point, player.zone.map.mapWidth)) {
                                mobs.add(mob);
                            }
                        }
                    }
                    int rangeBom = SkillUtil.getRangeBom(player.playerSkill.skillSelect.point, player.zone.map.mapWidth);
