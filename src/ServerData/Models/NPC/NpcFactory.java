...
import ServerData.Boss.ListBoss.Tau77;
...
public class NpcFactory {
                            ...
                            case 1:
                                ...
                                if (oldDuongTank != null) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Nhà ngươi không đủ 10k hồng ngọc và đạt tới nhiệm vụ 30");
                                } else {

                                    BossData bossDataClone = new BossData(
                                            "Bé rồng do" + " " + player.name + " hộ tống",
                                            (byte) 2,
                                            new short[] { 1489, 1490, 1491, -1, -1, -1 }, // outfit {head, body, leg,
                                                                                          // bag, aura, eff}
                                            100000,
                                    ...
                                    try {
                                        DuongTank dt = new DuongTank(Util.createIdDuongTank((int) player.id),
                                                bossDataClone, player.zone, player.location.x - 20, player.location.y);
                        ...
                        switch (select) {
                            ...
                            case 1:
                                Service.gI().sendPopUpMultiLine(player, tempId, avartar,
                                        "Khi đánh quái ở tất cả các map có tỉ lệ rơi vàng ngẫu nhiên, đánh boss cũng có tỉ lệ rơi ra thỏi vàng");
                                break;
                            case 2:
                                Service.gI().sendPopUpMultiLine(player, tempId, avartar,
                                        "Nhập Code, Đánh Boss để nhận hồng ngọc miễn phí");
                                break;
                            case 3:
                                Service.gI().sendPopUpMultiLine(player, tempId, avartar,
                            ...
                            case 4:
                                Service.gI().sendPopUpMultiLine(player, tempId, avartar,
                                        "Phó Bản Hiện Tại Đang Cập Nhật");
                                break;
                            case 5:
                            ...
                            case 6:
                                Service.gI().sendPopUpMultiLine(player, tempId, avartar,
                                        "Có 6 loại đệ tử, 3 loại ở Shop, 3 loại săn broly, 3 loại ở shop là cumber (30% chỉ số), vegeta ssj4 (50% chỉ số), goku ssj4 (50% chỉ số)"
                                                + " Đệ săn từ broly gồm mabu mập (20), ra đảo kame thức tỉnh đệ mabu mập có tỉ lệ ra 2 đệ sau bư han(25%), bư gầy (45%)");
                                break;
                            case 7:
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (this.mapId == 104 || this.mapId == 5) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                "|6|Bạn muốn đổi hồn linh thú cho mình sao?:\b|7|Cần có X99 Hồn Linh Thú + 5 thỏi vàng",
                                "Đổi Trứng\nLinh thú", "Từ chối");
                    }
                }
            }
                    ...
                    if (this.mapId == 113) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 1:
                                    this.createOtherMenu(player, 1,
                                            "|7|Ngươi ở top " + player.rankSieuHang
                                                    + " ,Mỗi ngày sẽ chốt Top giải siêu hạng một lần !",
                                            "Xem \nGiải Thưởng");
                                    break;
                                case 2:
                                    try {
                            ...
                            switch (select) {
                                ...
                                // Instant.ofEpochMilli(player.getSession().timesieuhang).atZone(ZoneId.systemDefault()).toLocalDate();
                                //
                                // if (lastClaimDate.isEqual(currentDate)) {
                                // Service.gI().sendThongBao(player, "Bạn đã nhận thưởng rồi trong ngày hôm
                                // nay");
                                // } else {
                                //
    ...
    // private static Npc maygap(int mapId, int status, int cx, int cy, int tempId,
    // int avartar) {
    // return new Npc(mapId, status, cx, cy, tempId, avartar) {
    // @Override
    // public void openBaseMenu(Player player) {
    ...
    // ĐƯỢC ĐẨY VÀO RƯƠNG PHỤ NẾU HÀNH TRANG ĐẦY!",
    ...
    // THƯỜNG : 5-10% CHỈ SỐ\n|3|GẮP X1 : GẮP THỦ CÔNG\nGẮP X10 : AUTO X10 LẦN
    // GẮP\nGẮP X100 : AUTO X100 LẦN GẮP\n"+"|7|LƯU Ý : MỌI CHỈ SỐ ĐỀU RANDOM KHÔNG
    ...
    // : AUTO X100 LẦN GẮP\n"+"|7|LƯU Ý : MỌI CHỈ SỐ ĐỀU RANDOM KHÔNG CÓ OPTION NHẤT
    // ĐỊNH\nNẾU MUỐN NGƯNG AUTO GẤP CHỈ CẦN THOÁT GAME VÀ VÀO LẠI!",
    // "Gắp x1","Gắp x10","Gắp x100","Rương Đồ");
    // break;
    // case 3: //
    ...
    // break;
    // case 4:
    // Service.gI().sendThongBaoFromAdmin(player, "Số điểm đã gắp của bạn :
    // "+player.gtPoint);
    // break;
    // case 5:
    ...
    // switch (select) {
    // case 0:
    // Item xuthuong = InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386);
    // if (xuthuong == null) {
    // this.createOtherMenu(player, 12345, "|2|HẾT TIỀN!\n|7|CẦN TỐI THIỂU 1 XU GẮP
    // THÚ, HÃY QUAY LẠI SAU!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    // }
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // this.createOtherMenu(player, 12345, "|2|Bạn vừa gắp được :
    ...
    // break;
    // case 1:
    // if (InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386) ==
    // null) {
    // this.createOtherMenu(player, 12345, "|2|HẾT TIỀN!\n|7|CẦN TỐI THIỂU 1 XU GẮP
    // THÚ, HÃY QUAY LẠI SAU!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    ...
    // InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386),1);
    // InventoryServiceNew.gI().sendItemBags(player);
    // if (InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386) ==
    // null) {
    ...
    // this.createOtherMenu(player, 12345, "|7|DỪNG AUTO GẮP, RƯƠNG PHỤ ĐÃ
    // ĐẦY!\n"+"|2|TỔNG LƯỢT GẮP : "+count+" LƯỢT"+"\n|7|VUI LÒNG LÀM TRỐNG RƯƠNG
    // PHỤ!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    // }
    ...
    // còn : "+InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity+"\n|7|TỔNG ĐIỂM : "+player.gtPoint+"\nNẾU HÀNH TRANG ĐẦY, ITEM
    // SẼ ĐƯỢC THÊM VÀO RƯƠNG PHỤ",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // } else {
    // this.createOtherMenu(player, 12345, "|7|ĐANG TIẾN HÀNH GẮP AUTO X10\nSỐ LƯỢT
    // CÒN : "+timex10+" LƯỢT\n"+"|2|Gắp hụt rồi!"+"\nSố xu còn :
    ...
    // switch (select) {
    // case 0:
    // Item xucc = InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386);
    // if (xucc == null || xucc.quantity < 3) {
    // this.createOtherMenu(player, 12346, "|2|HẾT TIỀN!\n|7|CẦN TỐI THIỂU 3 XU GẮP
    // THÚ, HÃY QUAY LẠI SAU!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    // }
    ...
    // xu còn : "+xucc.quantity+"\n|7|Chiến tiếp ngay!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // }
    // if(Util.isTrue(1, 300)) {
    // InventoryServiceNew.gI().addItemBag(player,
    // ItemService.gI().createNewItem((short)1384));
    // InventoryServiceNew.gI().sendItemBags(player);
    // this.createOtherMenu(player, 12346, "|6|Gắp hụt rồi, bạn bỏ cuộc sao?"+"\nSố
    // xu còn : "+xucc.quantity+"\n|7|Chiến tiếp ngay!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // player.gtPoint += 1;
    // this.createOtherMenu(player, 12346, "|2|Bạn vừa gắp được :
    ...
    // case 1:
    // if (InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386) ==
    // null || InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity < 3) {
    // this.createOtherMenu(player, 12346, "|2|HẾT TIỀN!\n|7|CẦN TỐI THIỂU 3 XU GẮP
    // THÚ, HÃY QUAY LẠI SAU!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    ...
    // count++;
    // InventoryServiceNew.gI().subQuantityItemsBag(player,
    // InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386),3);
    // InventoryServiceNew.gI().sendItemBags(player);
    // if (InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386) ==
    // null || InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity < 3) {
    ...
    // }
    // if(1+player.inventory.itemsBoxCrackBall.size() > 200) {
    // this.createOtherMenu(player, 12346, "|7|DỪNG AUTO GẮP, RƯƠNG PHỤ ĐÃ
    // ĐẦY!\n"+"|2|TỔNG LƯỢT GẮP : "+count+" LƯỢT"+"\n|7|VUI LÒNG LÀM TRỐNG RƯƠNG
    // PHỤ!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // player.gtPoint += 1;
    // this.createOtherMenu(player, 12346, "|7|ĐANG TIẾN HÀNH GẮP AUTO CAO CẤP
    // X10\nSỐ LƯỢT CÒN : "+timex10+" LƯỢT\n"+"|2|Đã gắp được :
    // "+gapx10.template.name+"\nSố xu còn :
    // "+InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity+"\n|7|TỔNG ĐIỂM : "+player.gtPoint+"\nNẾU HÀNH TRANG ĐẦY, ITEM
    ...
    // AUTO CAO CẤP X10 VÀO RƯƠNG PHỤ\nSỐ LƯỢT CÒN : "+timex10+" LƯỢT\n"+"|2|Đã gắp
    // được : "+gapx10.template.name+"\nSố xu còn :
    // "+InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity+"\n|7|TỔNG ĐIỂM : "+player.gtPoint,
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // } else {
    ...
    // player.inventory.itemsBoxCrackBall.add(gapx100);
    // this.createOtherMenu(player, 12346, "|7|HÀNH TRANG ĐÃ ĐẦY\nĐANG TIẾN HÀNH GẮP
    // AUTO CAO CẤP X100 VÀO RƯƠNG PHỤ\nSỐ LƯỢT CÒN : "+timex100+" LƯỢT\n"+"|2|Đã
    // gắp được : "+gapx100.template.name+"\nSố xu còn :
    // "+InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity+"\n|7|TỔNG ĐIỂM : "+player.gtPoint,
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // } else {
    ...
    // switch (select) {
    // case 0:
    // Item xuvip = InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386);
    // if (xuvip == null || xuvip.quantity < 5) {
    // this.createOtherMenu(player, 12347, "|2|HẾT TIỀN!\n|7|CẦN TỐI THIỂU 5 XU GẮP
    // THÚ, HÃY QUAY LẠI SAU!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    // }
    ...
    // count++;
    // InventoryServiceNew.gI().subQuantityItemsBag(player,
    // InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386),5);
    // InventoryServiceNew.gI().sendItemBags(player);
    // if (InventoryServiceNew.gI().findItem(player.inventory.itemsBag, 1386) ==
    // null || InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity < 3) {
    ...
    // }
    // if(1+player.inventory.itemsBoxCrackBall.size() > 200) {
    // this.createOtherMenu(player, 12347, "|7|DỪNG AUTO GẮP, RƯƠNG PHỤ ĐÃ
    // ĐẦY!\n"+"|2|TỔNG LƯỢT GẮP : "+count+" LƯỢT"+"\n|7|VUI LÒNG LÀM TRỐNG RƯƠNG
    // PHỤ!",
    // "Gắp X1","Gắp X10","Gắp X100","Rương Đồ");
    // break;
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // player.gtPoint += 1;
    // this.createOtherMenu(player, 12347, "|7|ĐANG TIẾN HÀNH GẮP AUTO CAO CẤP
    // X10\nSỐ LƯỢT CÒN : "+timex10+" LƯỢT\n"+"|2|Đã gắp được :
    // "+gapx10.template.name+"\nSố xu còn :
    // "+InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 1386).quantity+"\n|7|TỔNG ĐIỂM : "+player.gtPoint+"\nNẾU HÀNH TRANG ĐẦY, ITEM
                    ...
                    switch (select) {
                        case 0:
                            Service.getInstance().sendPopUpMultiLine(pl, tempId, avartar,
                                    "Cửa hàng chuyên nhận ký gửi mua bán vật phẩm\bChỉ với 3000 hồng ngọc\bGiá trị ký gửi 1-1000 thỏi vàng hoặc 1-100k hồng ngọc\bMột người bán, vạn người mua, mại dô, mại dô");
                            break;
                        case 1:
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0:
                                    if (player.nhanban >= 5) {
                                        Service.gI().sendThongBao(player,
                                                "Đã quá lượt gọi nhân bản trong ngày hôm nay");
                                        return;
                                    }
                ...
                if (canOpenNpc(player)) {
                    if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                        if (player.getSession().is_gift_box) {
                            ...
                            this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                    "Chào con, Ta có thể hỗ trợ nhiệm vụ 11, 13, 27",
                                    "Kho Báu\nDưới Biển", "Chức Năng\nBang Hội", "Hỗ Trợ\nNhiệm Vụ",
                                    "Nhận Quà\nTích Luỹ", "Hồi đến\n80% Skill", "Đóng");
                        }
                    }
                ...
                if (canOpenNpc(player)) {
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                                ...
                                switch (player.playerTask.taskMain.id) {
                                    case 11:
                                        switch (player.playerTask.taskMain.index) {
                                            case 0:
                                                ...
                                                break;
                                            case 1:
                                                for (int i = player.playerTask.taskMain.subTasks
                                                        .get(player.playerTask.taskMain.index).count; i < 25; i++) {
                                    ...
                                    case 27:
                                        switch (player.playerTask.taskMain.index) {
                                            case 0:
                                                for (int i = player.playerTask.taskMain.subTasks
                                                        .get(player.playerTask.taskMain.index).count; i < 15; i++) {
                                            ...
                                            default:
                                                Service.getInstance().sendThongBao(player,
                                                        "Nhiệm vụ hiện tại không thuộc diện hỗ trợ");
                                                break;
                                        }
                                        break;
                                    default:
                            ...
                            case 3:
                                createOtherMenu(player, 0,
                                        "Xin chào\n|2|Hiện tại con đã nạp : " + Util.format(player.getSession().tongnap)
                                                + " VNĐ"
                                                + "\nVà đang có: " + player.pointSb + " Điểm Săn Boss"
                                                + "\n|5|Đã nhận quà mốc nạp: "
                                                + (player.mocnap > 0 ? Util.format(player.mocnap) + " VNĐ"
                                                        : "Hiện chưa nhận mốc nạp nào")
                                                + "\nĐã nhận mốc săn boss: "
                                                + (player.mocsanboss > 0 ? Util.format(player.mocsanboss) + " Điểm"
                                                        : "Hiện chưa nhận mốc điểm nào")
                        ...
                        switch (select) {
                            case 0:
                                if (InventoryServiceNew.gI().findItemBag(player, 457).isNotNullItem()
                                        && InventoryServiceNew.gI().findItemBag(player, 457).quantity >= 0) {
                                    InventoryServiceNew.gI().subQuantityItemsBag(player,
                                            InventoryServiceNew.gI().findItemBag(player, 457), 0);
                                    InventoryServiceNew.gI().sendItemBags(player);
                                    int subTimeParam = 100;
                        ...
                        switch (select) {
                            case 0:
                                createOtherMenu(player, 1,
                                        "Xin chào"
                                                + "\n|2|Hiện tại con đã nạp được: "
                                                + Util.format(player.getSession().tongnap) + " VNĐ"
                            ...
                            case 0:
                                if (player.inventory.gold < 100000000) {
                                    ...
                                    Service.getInstance().sendThongBaoOK(player,
                                            notify.replace("con", player.name.toUpperCase()));
                                }
                                break;
                        }
                    }
                        ...
                        switch (select) {
                            ...
                            case 2:
                                this.createOtherMenu(player, 0, "|7|Kích Hoạt Tài Khoản\n"
                                        ...
                                        + (player.getSession().actived == 0 ? "Chưa kích hoạt" : "Đã kích hoạt")
                                        + "\n|2|Trạng thái VIP : "
                                        + (player.vip == 1 ? "VIP"
                                                : player.vip == 2 ? "VIP2"
                                                        : player.vip == 3 ? "VIP3"
                                    ...
                                    if (player.getSession().actived == 0) {
                                        if (player.inventory.ruby >= 10000) {
                                            try {
                                                ...
                                                Service.gI().sendThongBaoOK(player,
                                                        "Kích Hoạt Thành Công, Đăng Nhập Lại Để Kích Hoạt\nHệ Thống Sẽ Tự Kick Bạn Sau 5s");
                                            } catch (Exception e) {
    ...
    // public static Npc bulmaQK(int mapId, int status, int cx, int cy, int tempId,
    // int avartar) {
    // return new Npc(mapId, status, cx, cy, tempId, avartar) {
    // @Override
    // public void openBaseMenu(Player player) {
    ...
    // ShopServiceNew.gI().opendShop(player, "BUNMA", true);
    // } else {
    // this.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Xin lỗi cưng, chị chỉ bán
    // đồ cho người Trái Đất", "Đóng");
    // }
    // break;
    // }
    // }
            ...
            public void openBaseMenu(Player pl) {
                if (canOpenNpc(pl)) {
                    if (this.mapId == 84) {
                        ...
                        if (pl.playerTask.taskMain.id == 5) {
    ...
    // public static Npc bulmabip(int mapId, int status, int cx, int cy, int tempId,
    // int avartar) {
    // return new Npc(mapId, status, cx, cy, tempId, avartar) {
    // @Override
    // public void openBaseMenu(Player player) {
    // createOtherMenu(player, 0, "\b|8|Trò chơi Tài Xỉu đang được diễn ra\n\n|6|Thử
    // vận may của bạn với trò chơi Tài Xỉu! Đặt cược và dự đoán đúng"
    // + "\n kết quả, bạn sẽ được nhận thưởng lớn. Hãy tham gia ngay và\n cùng trải
    // nghiệm sự hồi hộp, thú vị trong trò chơi này!"
    // + "\n\n|7|(Điều kiện tham gia : mở thành viên)\n\n|2|Đặt tối thiểu: 1.000
    // Hồng ngọc\n Tối đa: 1.000.000.000.000 Hồng ngọc"
    // + "\n\n|7| Lưu ý : Thoát game khi chốt Kết quả sẽ MẤT Tiền cược và Tiền
    // thưởng", "Thể lệ", "Tham gia");
    // }
    //
    // @Override
    // public void confirmMenu(Player pl, int select) {
    // if (canOpenNpc(pl)) {
    // String time = ((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) + " giây";
    // if (pl.iDMark.getIndexMenu() == 0) {
    // if (select == 0) {
    // createOtherMenu(pl, ConstNpc.IGNORE_MENU, "|5|Có 2 nhà cái Tài và Xĩu, bạn
    // chỉ được chọn 1 nhà để tham gia"
    // + "\n\n|6|Sau khi kết thúc thời gian đặt cược. Hệ thống sẽ tung xí ngầu để
    // biết kết quả Tài Xỉu"
    // + "\n\nNếu Tổng số 3 con xí ngầu <=10 : XỈU\nNếu Tổng số 3 con xí ngầu >10 :
    // TÀI\nNếu 3 Xí ngầu cùng 1 số : TAM HOA (Nhà cái lụm hết)"
    // + "\n\n|7|Lưu ý: Số Hồng ngọc nhận được sẽ bị nhà cái lụm đi 20%. Trong quá
    // trình diễn ra khi đặt cược nếu thoát game trong lúc phát thưởng phần quà sẽ
    // bị HỦY", "Ok");
    // } else if (select == 1) {
    // if (TaiXiu_Old.gI().baotri == false) {
    // if (pl.goldTai == 0 && pl.goldXiu == 0) {
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    // TaiXiu_Old.gI().z
    // + "\n\n|6|Tổng nhà TÀI: " + Util.format(TaiXiu_Old.gI().goldTai) + " Hồng
    // ngọc"
    // + "\n|1|Tổng người đặt TÀI: " + TaiXiu_Old.gI().PlayersTai.size() + " người"
    // + "\n\n|6|Tổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
    // ngọc"
    ...
    // createOtherMenu(pl, 1, "\n|7|---NHÀ CÁI TÀI XỈU---\n\n|3|Kết quả kì trước: "
    // + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " + TaiXiu_Old.gI().z
    // + "\n\n|6|Tổng nhà TÀI: " + Util.format(TaiXiu_Old.gI().goldTai) + " Hồng
    // ngọc"
    // + "\n|1|Tổng người đặt TÀI: " + TaiXiu_Old.gI().PlayersTai.size() + " người"
    // + "\n\n|6|Tổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
    ...
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    // TaiXiu_Old.gI().z + "\n\n|6|Tổng nhà TÀI: " +
    // Util.format(TaiXiu_Old.gI().goldTai) + " Hồng ngọc"
    // + "\n\nTổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
    // ngọc\n\n|5|Thời gian còn lại: " + time + "\n\n|7|Bạn đã cược Xỉu : " +
    // Util.format(pl.goldXiu) + " Hồng ngọc" + "\n\n|7|Hệ thống sắp bảo trì", "Cập
    // nhập", "Đóng");
    // }
    // }
    // }
    ...
    // && pl.goldTai == 0 && pl.goldXiu == 0 && TaiXiu_Old.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    ...
    // Service.gI().sendThongBao(pl, "Vui lòng kích hoạt tài khoản và đạt nhiệm vụ
    // 26 để sử dụng chức năng này");
    // } else {
    ...
    // break;
    // }
    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) > 0 && pl.goldTai > 0 && TaiXiu_Old.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    ...
    // }
    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) > 0 && pl.goldXiu > 0 && TaiXiu_Old.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    ...
    // }
    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) > 0 && pl.goldTai > 0 && TaiXiu_Old.gI().baotri == true) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    ...
    // }
    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) > 0 && pl.goldXiu > 0 && TaiXiu_Old.gI().baotri == true) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    // quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + " : " +
    ...
    // }
    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
    // 1000) > 0 && pl.goldXiu == 0 && pl.goldTai == 0 && TaiXiu_Old.gI().baotri ==
    // true) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|7|---ĐỠ THẾ ĐÉO NÀO ĐƯỢC CÁC ÔNG À---\n\n|3|Kết
    ...
    // public static Npc minuongevent(int mapId, int status, int cx, int cy, int
    // tempId, int avartar) {
    // return new Npc(mapId, status, cx, cy, tempId, avartar) {
    // @Override
    // public void openBaseMenu(Player player) {
    ...
    // break;
    // case 2:
    // this.createOtherMenu(player, ConstEvent.HUNGVUONG, "|7|-Sự Kiện Hùng
    // Vương-\n"
    ...
    // + "Chuẩn bị x10 nguyên liệu Kẹo, Bánh Quy, Bí ngô để đổi Giỏ Kẹo cho ta
    // nhé\n"
    // + "Nguyên Liệu thu thập bằng cách đánh quái tại các hành tinh được chỉ
    // định\n"
    // + "Tích lũy 3 Giỏ Kẹo + 3 Vé mang qua đây ta sẽ cho con 1 Hộp Ma Quỷ\n"
    // + "Tích lũy 3 Giỏ Kẹo, 3 Hộp Ma Quỷ + 3 Vé \nmang qua đây ta sẽ cho con 1 hộp
    // quà thú vị.",
    // "Đổi\nGiỏ Kẹo", "Đổi Hộp\nMa Quỷ", "Đổi Hộp\nQuà Halloween",
    // "Từ chối");
    // break;
    ...
    // + "5" + " phút."
    // + "\nKhi tặng đủ 999 bông hoa toàn bộ máy chủ được nhân đôi số vàng rơi ra từ
    // quái trong 60 phút",
    // "Tặng 1\n Bông hoa", "Tặng\n10 Bông", "Tặng\n99 Bông",
    // "Đổi\nHộp quà");
    // break;
    ...
    // + "\n" + (ConstEvent.TONGSOBANH >= 500 ? "|7|HIỆN TẠI ĐANG X2 EXP TRÊN TOÀN
    // MÁY CHỦ\nTHỜI GIAN CÒN: " + Util.tinhgio(ConstEvent.X2) : "|7|Tổng số bánh
    // server: " + ConstEvent.TONGSOBANH
    ...
    // + "\n\n|5|Ăn bánh trung thu nhận được Điểm tích lũy Sự kiện Trung thu đổi
    // được các phần quà hấp dẫn"
    ...
    // + "|2|Mốc 500 Điểm\n"
    // + "|4|200 Mảnh thiên sứ ngẫu nhiên, 20 rương thần linh, 15 Hộp quà Trung thu,
    // 10 Thẻ gia hạn, 1 Phiếu giảm giá + 250k HN \n"
    // + "|2|Mốc 300 Điểm\n"
    // + "|4|100 Mảnh thiên sứ ngẫu nhiên, 15 rương thần linh, 10 Thẻ gia hạn, 15
    // Hộp Trung thu + 150k HN \n"
    // + "|2|Mốc 200 Điểm\n"
    // + "|4|50 Mảnh thiên sứ ngẫu nhiên, 10 rương thần linh, 5 Thẻ gia hạn, 10 Hộp
    // Trung thu + 100k HN \n"
    // + "|2|Mốc 50 Điểm\n"
    // + "|4|10 Mảnh thiên sứ ngẫu nhiên, 5 rương thần linh + 25k HN"
    // + "\n\n|7|Điểm sự kiện : " + Util.format(player.inventory.coupon) + " Điểm"
    // + "\n|1|Điểm Top Trung thu : " + Util.format(player.inventory.event) + "
    // Điểm",
    ...
    // + ", 20 Rương thần linh, 15 Hộp Trung thu, 30 Thẻ gia hạn, 1 Phiếu giảm giá
    // và 250k Hồng ngọc");
    // }
    // break;
    // }
    ...
    // + ", 15 Rương thần linh, 15 Hộp Trung thu, 10 Thẻ gia hạn và 150k Hồng
    // ngọc");
    // }
    // break;
    // }
    ...
    // } else {
    // Service.getInstance().sendThongBao(player,
    // "Vui lòng chuẩn bị x10 Nguyên Liệu Kẹo, Bánh Quy, Bí Ngô để đổi vật phẩm sự
    // kiện");
    // }
    // } else {
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // Service.getInstance().sendThongBao(player, "Bạn nhận được Hộp Quà Teacher
    // Day");
    // } else {
    ...
    // Service.getInstance().sendThongBao(player, "Bạn nhận được " + n + " điểm sự
    // kiện");
    // } else {
    // Service.getInstance().sendThongBao(player,
    ...
    // }
    // } else {
    // Service.getInstance().sendThongBao(player, "Cần ít nhất " + n + " bông hoa để
    // có thể tặng");
    // }
    // }
    // break;
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // Service.getInstance().sendThongBao(player, "Bạn nhận được Tất,vớ giáng
    // sinh");
    // } else {
    // Service.getInstance().sendThongBao(player,
    ...
    // slhn + " hồng ngọc\nx" + sltv + " thỏi vàng");
    // Service.getInstance().sendThongBao(player, "Nhận lì xì thành công,chúc bạn
    // năm mới dui dẻ");
    // } else if (day > 15) {
    ...
    // break;
    // case 3:
    // if (InventoryServiceNew.gI().findItemBag(player, 1436) == null &&
    // InventoryServiceNew.gI().findItemBody(player, 1436) == null) {
            ...
            public void openBaseMenu(Player player) {
                if (this.mapId == 5) {
                    if (ConstEvent.EVENT == 7) {
                        if (player.BanhChung > 0 && NauBanh.gI().NauXong == false
                                || player.BanhTet > 0 && NauBanh.gI().NauXong == false
                                || player.BanhChung > 0 && player.BanhTet > 0 && NauBanh.gI().NauXong == false) {
                            this.createOtherMenu(player, 0, "|2|Nồi Bánh Toàn Server Đợt " + NauBanh.Count
                                    + "\n|-1|Đang đang có bánh chưa nhận"
                                    + "\nBạn đã nấu: " + player.BanhChung + " Bánh Chưng và " + player.BanhTet
                                    + " Bánh Tét"
                                    + "\nHãy nhận bánh trước!",
                                    "Lấy Bánh");
                            ...
                            this.createOtherMenu(player, 1, "|2|Nồi Bánh Toàn Server Đợt " + NauBanh.Count
                                    ...
                                    + ((player.BanhChungDaNau + player.BanhTetDaNau) > 0
                                            ? "\nTrong đó bạn có: " + (player.BanhChungDaNau + player.BanhTetDaNau)
                                                    + " bánh mới"
                                            : "")
                                    ...
                                    + (NauBanh.Nuoc >= 100 && NauBanh.Nuoc < 300 ? "(Thấp)"
                                            : NauBanh.Nuoc >= 300 && NauBanh.Nuoc < 500 ? "(Trung Bình)"
                                                    : NauBanh.Nuoc >= 500 ? "(Đã đầy)" : "(Rất ít)")
                                ...
                                switch (select) {
                                    case 0:
                                        this.createOtherMenu(player, 2,
                                                "Bánh chưng: 12 Lá giong, 2 Gạo nếp, 1 Đậu xanh, 12 Gióng tre, 3 Thịt lợn, 1 Muối và 03 Nước nấu.",
                                                "Nấu", "Đóng");
                                        break;
                                    case 1:
                                        this.createOtherMenu(player, 3,
                                                "Bánh tết: 12 Lá chuối, 2 Gạo nếp, 1 Đậu xanh, 12 Giống tre, 3 Thịt lợn, 1 Muối và 03 Nước nấu.",
                                                "Nấu", "Đóng");
                                        break;
                                }
                                break;
                                ...
                                switch (select) {
                                    case 0:
                                        this.createOtherMenu(player, 5, "|2|Nồi Bánh Toàn Server Đợt " + NauBanh.Count
                                                + "\n|-1|Đang trong thời gian chờ"
                                                + "\n|7|Thời gian chờ còn: " + Util.tinhgio(NauBanh.ThoiGianCho)
                                                + "\n|-1|Mực nước trong nồi: " + Util.format(NauBanh.Nuoc / 500 * 100)
                                                + "/100% "
                                                + (NauBanh.Nuoc >= 100 && NauBanh.Nuoc < 300 ? "(Thấp)"
                                                        : NauBanh.Nuoc >= 300 && NauBanh.Nuoc < 500 ? "(Trung Bình)"
                                                                : NauBanh.Nuoc >= 500 ? "(Đã đầy)" : "(Rất ít)")
                    ...
                    if (this.mapId == 5) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 2:
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.PHA_LE_HOA_TRANG_BI_VIP);
                                    break;
                          ...
                          // break;
                            ...
                            switch (select) {
                                ...
                                case 1:
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.PHAN_RA_DO_THAN_LINH_X3);
                                    break;
                                case 2:
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.PHAN_RA_DO_THAN_LINH_X5);
                                    break;
                        ...
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 2: // nâng cấp bông tai
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.NANG_CAP_BONG_TAI_1_LEN_2);
                                    break;
                                case 3: //
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.MO_CHI_SO_BONG_TAI_CAP2);
                                    break;
                                case 4: // nâng cấp bông tai
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.NANG_CAP_BONG_TAI_2_LEN_3);
                                    break;
                                case 5: //
                                    CombineServiceNew.gI().openTabCombine(player,
                                ...
                                case 6: // nâng cấp bông tai
                                    CombineServiceNew.gI().openTabCombine(player,
                                            CombineServiceNew.NANG_CAP_BONG_TAI_3_LEN_4);
                                    break;
                                case 7: //
                                    CombineServiceNew.gI().openTabCombine(player,
                                ...
                                // CombineServiceNew.gI().openTabCombine(player,
                                // CombineServiceNew.NANG_CAP_BONG_TAI_CAP5);
                                // break;
                                // case 10: //
                                // CombineServiceNew.gI().openTabCombine(player,
                                // CombineServiceNew.MO_CHI_SO_BONG_TAI_CAP5);
                                // break;
                            }
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (mapId == 5 || mapId == 0 || mapId == 7 || mapId == 14) {
                        try {
                            ...
                            if (this.mapId == 5 || mapId == 0 || mapId == 7 || mapId == 14) {
                                ...
                                if (now2 > MapMaBu.TIME_OPEN_TN && now2 < MapMaBu.TIME_CLOSE_TN
                                        || player.itemTime.VENGOC == true) {
                                    this.createOtherMenu(player, ConstNpc.MENU_OPEN_TN,
                                            "|1|Map Up Tiềm Năng đã mở cửa, tham gia ngay?",
                                            "Chiến Ngay", "XEM TOP");
                                } else if (now2 > MapMaBu.TIME_OPEN_TN2
                                        && now2 < MapMaBu.TIME_CLOSE_TN2 | player.itemTime.VENGOC == true) {
                                    this.createOtherMenu(player, ConstNpc.MENU_OPEN_TN2,
                                            "|1|Map Up Tiềm Năng đã mở cửa, tham gia ngay?",
                                            "Chiến Ngay", "XEM TOP");
                                } else {
                                    this.createOtherMenu(player, ConstNpc.MENU_NOT_OPEN_TN,
                                            "|7|HIỆN CHƯA ĐẾN THỜI GIAN MỞ MAP\n|2|Thời Gian Mở : 9-12H SÁNG và 9-12H TỐI\n"
                                                    ...
                                                    + "[ Khi Map Đóng, Chỉ Những Ai Đang Sử Dụng Vé Mới Có Thể Ở Lại ]",
                                            "Đóng", "Xem Top");
                                }
                            }
                        } catch (Exception ex) {
                    ...
                    if (mapId == 122 || mapId == 204) {
                        this.createOtherMenu(player, 0, "Xia xia thua phùa\b|7|Thí chủ đang có: " + player.nhsPoint
                                + " điểm ngũ hành sơn\b|1|Thí chủ có muốn đổi 500 điểm tu luyện của mình lấy cải trang không?",
                                "Đổi cải trang", "Top Ngũ Hành Sơn", "Không");
                    }
                }
            }
                    ...
                    if (this.mapId == 154) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 1:
                                    if (player.gender == 1) {
                                        this.createOtherMenu(player, ConstNpc.HOC_SKILL_NM,
                                                "|7|Học Chiêu Siêu Cấp Namek\n"
                                                        + "|2|[ YÊU CẦU BAN ĐẦU ]\n"
                                                        + "|5|+ 60 tỉ tiềm năng\n+ 50.000 hồng ngọc\n+ 1 tỉ vàng\n"
                                                        + "|2|Mỗi cấp học sẽ tăng thêm 10 tỉ tiềm năng và 90% thành thạo!\n"
                                                        + "|7|Chọn Level Skill Muốn Học",
                                ...
                                case 2:
                                    if (player.gender == 2) {
                                        this.createOtherMenu(player, ConstNpc.HOC_SKILL_XD,
                                                "|7|Học Chiêu Siêu Cấp Xayda\n"
                                                        + "|2|[ YÊU CẦU BAN ĐẦU ]\n"
                                                        + "|5|+ 60 tỉ tiềm năng\n+ 50.000 hồng ngọc\n+ 1 tỉ vàng\n"
                                                        + "|2|Mỗi cấp học sẽ tăng thêm 10 tỉ tiềm năng và 90% thành thạo!\n"
                                                        + "|7|Chọn Level Skill Muốn Học",
    ...
    // createOtherMenu(player, 0, "\b|8|Trò chơi Chẵn Lẻ may rủi đang diễn
    // ra\n\n|6|Trò chơi sẽ chọn ra chỉ duy nhất 1 người may mắn nhất giành chiến
    // thắng để trao giải, giải thưởng sẽ là tổng thỏi vàng tất cả người chơi đặt
    // vào trong thời gian diễn ra giải.\n\n|2|ĐẶT CHẴN : 20 Thỏi Vàng/1 Lượt\nĐẶT
    // LẺ : 200 Thỏi Vàng/ 1 Lượt\n\n|7|Nếu bạn tin tưởng mình đang tràn đầy may
    ...
    // ChonAiDay.gI().baotri == false) {
    // if (select == 0) {
    // createOtherMenu(pl, ConstNpc.IGNORE_MENU, "|8|Thời gian giữa các giải là 5
    // phút\n|2|Khi hết giờ, hệ thống sẽ ngẫu nhiên chọn ra 1 người may mắn.\n|5|Lưu
    // ý: Số thỏi vàng nhận được sẽ bị nhà cái lụm đi 5%!\n|7|Trong quá trình diễn
    // ra khi đặt cược nếu thoát game mọi phần đặt đều sẽ bị hủy", "Ok");
    // } else if (select == 1) {
    // if (pl.goldNormar > 0) {
    // createOtherMenu(pl, 2, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
    // pl.percentGold(0) + "%\nTổng giải LẺ: " + ChonAiDay.gI().goldVip + " thỏi
    // vàng, cơ hội trúng của bạn là: " + pl.percentGold(1) + "%\n\n|2|Số thỏi vàng
    // ĐẶT CHẴN: " + pl.goldNormar + "\n\n|5|Thời gian còn lại: " + time,
    // "Cập nhập", "ĐẶT CHẴN\n5TV", "ĐẶT CHẴN\n10TV", "ĐẶT CHẴN\n20TV", "ĐẶT
    // CHẴN\n50TV", "Đóng");
    // }
    // if (pl.goldVIP > 0) {
    ...
    // } else {
    // if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) > 0 &&
    // ChonAiDay.gI().baotri == true) {
    // if (select == 0) {
    // createOtherMenu(pl, ConstNpc.IGNORE_MENU, "|8|Thời gian giữa các giải là 5
    // phút\n|2|Khi hết giờ, hệ thống sẽ ngẫu nhiên chọn ra 1 người may mắn.\n|5|Lưu
    // ý: Số thỏi vàng nhận được sẽ bị nhà cái lụm đi 5%!Trong quá trình diễn ra khi
    // đặt cược nếu thoát game mọi phần đặt đều sẽ bị hủy", "Ok");
    // } else if (select == 1) {
    // createOtherMenu(pl, 1, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
    // pl.percentGold(0) + "%\nTổng giải LẺ: " + ChonAiDay.gI().goldVip + " thỏi
    ...
    // } else if (pl.iDMark.getIndexMenu() == 1) {
    // if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) > 0 &&
    // pl.goldVIP == 0 && pl.goldNormar == 0 && ChonAiDay.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
    ...
    // if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) > 0 &&
    // pl.goldNormar >= 0 && ChonAiDay.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 2, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
    ...
    // InventoryServiceNew.gI().findItemBag(pl, 457), 5);
    ...
    // try {
    // if (InventoryServiceNew.gI().findItemBag(pl, 457).isNotNullItem() &&
    // InventoryServiceNew.gI().findItemBag(pl, 457).quantity >= 10) {
    // InventoryServiceNew.gI().subQuantityItemsBag(pl,
    // InventoryServiceNew.gI().findItemBag(pl, 457), 10);
    // InventoryServiceNew.gI().sendItemBags(pl);
    // pl.goldNormar += 10;
    ...
    // try {
    // if (InventoryServiceNew.gI().findItemBag(pl, 457).isNotNullItem() &&
    // InventoryServiceNew.gI().findItemBag(pl, 457).quantity >= 50) {
    // InventoryServiceNew.gI().subQuantityItemsBag(pl,
    // InventoryServiceNew.gI().findItemBag(pl, 457), 50);
    // InventoryServiceNew.gI().sendItemBags(pl);
    // pl.goldNormar += 50;
    ...
    // if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) > 0 &&
    // pl.goldVIP >= 0 && ChonAiDay.gI().baotri == false) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 3, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
    ...
    // try {
    // if (InventoryServiceNew.gI().findItemBag(pl, 457).isNotNullItem() &&
    // InventoryServiceNew.gI().findItemBag(pl, 457).quantity >= 5) {
    // InventoryServiceNew.gI().subQuantityItemsBag(pl,
    // InventoryServiceNew.gI().findItemBag(pl, 457), 5);
    // InventoryServiceNew.gI().sendItemBags(pl);
    // pl.goldVIP += 5;
    ...
    // break;
    // }
    // } else if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000)
    // > 0 && ChonAiDay.gI().baotri == true) {
    // switch (select) {
    // case 0:
    // createOtherMenu(pl, 1, "\n|8|- NHÀ CÁI CHẴN LẺ Kamui -\n\n|1|Tổng giải CHẴN:
    // " + ChonAiDay.gI().goldNormar + " thỏi vàng, cơ hội trúng của bạn là: " +
                                ...
                                case 0:
                                    if (player.clan.haveGoneConDuongRanDoc) {
                                        createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                                "Bang hội của ngươi đã đi con đường rắn độc lúc "
                                                        + TimeUtil.formatTime(player.clan.lastTimeOpenConDuongRanDoc,
                                                                "HH:mm:ss")
                                                        + " hôm nay. Người mở\n"
                                                        + "(" + player.clan.playerOpenConDuongRanDoc.name
                    ...
                    if (player.clan.doanhTrai != null) {
                        createOtherMenu(player, ConstNpc.MENU_JOIN_DOANH_TRAI,
                                "Bang hội của ngươi đang đánh trại độc nhãn\n"
                                        ...
                                        + TimeUtil.getMinLeft(player.clan.timeOpenDoanhTrai,
    ...
    public static Npc boMong(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    if (player.playerTask.taskMain.id == 9 && this.mapId == 47) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                ...
                                case 1:
                                    ...
                                    BossData bossDataClone = new BossData(
                                        ...
                                        player.nPoint.dame,
                                            ...
                                            new int[] {47},
                                            skillTemp,
                                            new String[] { "|-2|Ha Ha Ha Ha ... " }, // text chat 1
                                            new String[] { "|-1|Ngươi tưởng sẽ ăn được ta sao" }, // text
                                            new String[] { "|-1|Không thể nào" }, // text chat 3
                                            60);
                                    try {
                                        System.out.println(player.getHead() + " " + player.getBody() + " "+ player.getLeg());
                                        new Tau77(BossID.TAU_PAY_PAY, bossDataClone, player.zone);
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                                    break;
                                case 2:
                                    TaskService.gI().doneTask(player, ConstTask.TASK_9_0);
                                    TaskService.gI().doneTask(player, ConstTask.TASK_9_2);
                                    break;
                                case 3:
    ...
    /*
     * public static Npc karin(int mapId, int status, int cx, int cy, int tempId,
     * int avartar) {
     * return new Npc(mapId, status, cx, cy, tempId, avartar) {
     * 
     * @Override
     ...
     * if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
     * this.createOtherMenu(player, ConstNpc.BASE_MENU,
     * "|1|Cửa Hàng Của Ta Sưu Tầm Nhưng Đồ Vip Độc Lạ"
                    ...
                    if (this.mapId == 133 || this.mapId == 0) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                case 0: {
                                    Item biKiep = null;
                                    Item caitrangyardrat = ItemService.gI().createNewItem(
                                            (short) (player.gender == 0 ? 592 : player.gender == 1 ? 593 : 594));
                                    try {
                                    ...
                                    if (biKiep == null || biKiep.quantity < 9999) {
                                        ...
                                        caitrangyardrat.itemOptions.add(new Item.ItemOption(33, 1));// dich chuyen tuc
                                                                                                    // thoi
    ...
    // public static Npc gokuSSJ_2(int mapId, int status, int cx, int cy, int
    // tempId, int avartar) {
    // return new Npc(mapId, status, cx, cy, tempId, avartar) {
    // @Override
    // public void openBaseMenu(Player player) {
    ...
    // if (this.mapId == 133) {
    // try {
    // Item biKiep = InventoryServiceNew.gI().findItem(player.inventory.itemsBag,
    // 590);
    // if (biKiep != null) {
    // this.createOtherMenu(player, ConstNpc.BASE_MENU, "Bạn đang có " +
    // biKiep.quantity + " bí kiếp.\n"
    // + "Hãy kiếm đủ 10000 bí kiếp tôi sẽ dạy bạn cách dịch chuyển tức thời của
    // người Yardart", "Học dịch\nchuyển", "Đóng");
    // }
    //
    // } catch (Exception ex) {
    // ex.printStackTrace();
    ...
    // InventoryServiceNew.gI().sendItemBags(player);
    // Service.gI().sendThongBao(player, "Bạn vừa nhận được trang phục tộc
    // Yardart");
    // }
    // }
    // } catch (Exception ex) {
        ...
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                        ...
                        if (now > MapMaBu.TIME_OPEN_PVP && now < MapMaBu.TIME_CLOSE_PVP) {
                            this.createOtherMenu(player, 0,
                                    "Bạn muốn gì nào?\n|7|MAP ĐẤU TRƯỜNG PVP MỞ CỬA VÀO 20-22H MỖI NGÀY\n"
                                            ...
                                            + "\n|2|Bạn đang có : " + player.pointPvp + " điểm Đấu Trường",
                                    "Đến\nVõ Đài\nSiêu Cấp", "Đổi\nCải trang", "Top PvP");
                        } else {
                            this.createOtherMenu(player, 0,
                                    "Bạn muốn gì nào?\n|7|MAP ĐẤU TRƯỜNG PVP MỞ CỬA VÀO 20-22H MỖI NGÀY\n"
                    ...
                    if (this.mapId == 196) {
                        this.createOtherMenu(player, 0,
                                "Bạn muốn gì nào?\nBạn đang có : " + player.pointPvp + " điểm Đấu Trường",
                                "Về\nĐảo Kame");
                    }
                }
            }
                            ...
                            switch (select) {
                                case 0:
                                    if (player.playerTask.taskMain.id >= 25) {
                                        ...
                                        Service.getInstance().sendThongBao(player,
                                                "Thực lực của bạn chưa đủ để đến đây");
                                        break; // đến võ đài siêu cấp
                                    }
                                case 1: //
                                    this.createOtherMenu(player, 1,
                                ...
                                case 0: // trade
                                    if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                                        if (player.pointPvp >= 500) {
                                            ...
                                            InventoryServiceNew.gI().sendItemBags(player);
                                            Service.getInstance().sendThongBao(player,
                                                    "Đổi thành công, hãy thoát ra vào lại để cảm nhận!");
                                        } else {
                                            Service.getInstance().sendThongBao(player,
                ...
                if (canOpenNpc(player)) {
                    if (this.mapId == 5) {
                        this.createOtherMenu(player, ConstNpc.BANKING, "|7|- •⊹٭DragonBall Kamui٭⊹• -\n"
                                + "|2|Thông Tin Tổng\b" + "Chào bạn : " + player.name + " | ID: (" + player.id + ") | "
                ...
                if (canOpenNpc(player)) {
                    if (this.mapId == 5) {
                        if (player.iDMark.getIndexMenu() == ConstNpc.BANKING) {
                            switch (select) {
                                case 0:
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.QUY_DOI, 12639,
                                            "|7|DragonBall Kamui" + "\n"
                                                    + "ĐỔI TIỀN\n"
                                                    + "|2|Chào bạn : " + player.name
                                                    + "\nSố Dư : " + Util.format(player.getSession().vnd) + " VNĐ "
                                                    + "\n|7|Tỉ Lệ Quy Đổi Hiện Tại Là : Thỏi Vàng X" + Manager.TLDOITV
                                                    + ", Hồng Ngọc X" + Util.format(Manager.TLDOIHN) + ", Xu BẠC X"
                                                    + Util.format(Manager.TLDOIXUBAC) + ", Xu VÀNG X"
                                                    + Util.format(Manager.TLDOIXUVANG) + ", Thăng Tinh Thạch X"
                                                    + Util.format(Manager.TLDOITHANGTINHTHACH)
                                                    + "\n|5|(Giới Hạn Quy Đồi Là 1000 tức không quá 1 triệu vnđ)"
                                                    + "\n" + "Ví Dụ : Nhập vào 1 = 1.000VNĐ = " + Manager.TLDOITV
                                                    + " Thỏi Vàng "
                                                    + "\n" + "Ví Dụ : Nhập vào 1 = 1.000VNĐ = "
                                                    + Util.format(Manager.TLDOIHN * 1000) + " Hồng Ngọc"
                                                    + "\n" + "Ví Dụ : Nhập vào 1 = 1.000VNĐ = "
                                                    + Util.format(Manager.TLDOIXUVANG) + " XU Vàng"
                                                    + "\n" + "Ví Dụ : Nhập vào 1 = 1.000VNĐ = "
                                                    + Util.format(Manager.TLDOIXUBAC) + " XU Bạc"
                                                    + "\n" + "Ví Dụ : Nhập vào 1 = 1.000VNĐ = "
                                                    + Util.format(Manager.TLDOITHANGTINHTHACH) + " Thanh Tinh Thạch",
                                            "ĐỔI\nTHỎI VÀNG", "ĐỔI\nHỒNG NGỌC", "Đổi\n Xu Vàng", "Đổi\n Xu Bạc",
                                            "Đổi\nThăng\nTinhThach");
                                    break;
                                case 1:
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.MUA_DE, 12639,
                                            "|7|DragonBall Kamui" + "\n"
                                                    + "MUA ĐỆ TỬ\n"
                                                    + "|2|Chào bạn : " + player.name
                                                    + "\nSố Dư : " + Util.format(player.getSession().vnd) + " VNĐ\n"
                                    ...
                                    break;
                                case 2:
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.DOI_SKILL, 12639,
                                            "|7|DragonBall Kamui" + "\n"
                                                    + "ĐỔI SKILL ĐỆ TỬ\n"
                                                    + "|2|Chào bạn : " + player.name
                                                    + "\nSố Dư : " + Util.format(player.getSession().vnd) + " VNĐ\n"
                    ...
                    if (this.mapId == 52) {
                        ...
                        if (pl.levelWoodChest == 0) {
                            menuselect = new String[] { "Hướng\ndẫn\nthêm",
                                    ...
                                    "Thi đấu\n" + Util.numberToMoney(goldchallenge1) + " Ngọc",
                                    "Về\nĐại Hội\nVõ Thuật" };
                        } else {
                            menuselect = new String[] { "Hướng\ndẫn\nthêm",
                                    "Thi đấu\n" + Util.numberToMoney(goldchallenge) + " vàng",
                ...
                if (canOpenNpc(player) && this.mapId == 46) {
                    if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                        ...
                        if (player.clan != null && player.clan.ConDuongRanDoc != null && player.clan.gobosscdrd) {
                            this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                    "Hãy cầm lấy hai hạt đậu cuối cùng của ta đây\nCố giữ mình nhé " + player.name,
                                    "Cám ơn\nsư phụ");
            ...
            public void confirmMenu(Player player, int select) {
                OUTER: switch (player.iDMark.getIndexMenu()) {
                    ...
                    case ConstNpc.MENU_OPTION_USE_ITEM1498:
                        switch (select) {
                            case 0:
                                NpcService.gI().createMenuConMeo(player, 0, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 1:
                                NpcService.gI().createMenuConMeo(player, 1, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 2:
                                NpcService.gI().createMenuConMeo(player, 2, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                        }
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM1499:
                        switch (select) {
                            case 0:
                                NpcService.gI().createMenuConMeo(player, 3, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 1:
                                NpcService.gI().createMenuConMeo(player, 4, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 2:
                                NpcService.gI().createMenuConMeo(player, 5, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                        }
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM1500:
                        switch (select) {
                            case 0:
                                NpcService.gI().createMenuConMeo(player, 6, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 1:
                                NpcService.gI().createMenuConMeo(player, 7, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 2:
                                NpcService.gI().createMenuConMeo(player, 8, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                        }
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM1501:
                        switch (select) {
                            case 0:
                                NpcService.gI().createMenuConMeo(player, 9, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 1:
                                NpcService.gI().createMenuConMeo(player, 10, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                            case 2:
                                NpcService.gI().createMenuConMeo(player, 11, -1, "Hãy chọn một món quà", "Áo", "Quần",
                                        "Găng", "Giày", "Rada", "Từ Chối");
                                break;
                        }
                        break;
                        ...
                        if (select == 0) {
                            if (((Player) PLAYERID_OBJECT.get(player.id)).isAdmin2()) {
                                Service.gI().sendThongBaoOK((Player) PLAYERID_OBJECT.get(player.id),
                                        "Thằng [" + player.name + "] đang muốn ban mày");
                    ...
                    case ConstNpc.ACTIVE:
                        if (select == 0) {
                            ...
                            Service.gI().sendThongBao(player, "Active Player : "
                        ...
                        try {
                            if (select == 0) {
                                if (((Player) PLAYERID_OBJECT.get(player.id)).getSession().isJail == false) {
                                    ...
                                    ChangeMapService.gI().changeMap(((Player) PLAYERID_OBJECT.get(player.id)), 49, 0,
                                            620, 320);
                                    Service.gI().sendThongBaoOK(player, "Giam người chơi : "
                                            + ((Player) PLAYERID_OBJECT.get(player.id)).name + " thành công");
                                    Thread.sleep(3000);
                                    Service.gI().sendThongBaoFromAdmin(((Player) PLAYERID_OBJECT.get(player.id)),
                                            "Bạn Đã Bị Giam Bởi " + player.name);
                                    ServerNotify.gI().sendThongBaoBenDuoi(
                                            "người chơi : " + ((Player) PLAYERID_OBJECT.get(player.id)).name
                                                    + " đã vào tù vì phạm tội");
                                } else {
                                    PlayerService.gI().jail((Player) PLAYERID_OBJECT.get(player.id));
                                    ChangeMapService.gI().changeMap(((Player) PLAYERID_OBJECT.get(player.id)), 5, -1,
                                            1040, 169);
                                    Service.gI().sendThongBaoOK(player, "Mở giam người chơi : "
                                            + ((Player) PLAYERID_OBJECT.get(player.id)).name + " thành công");
                                    Thread.sleep(3000);
                                    Service.gI().sendThongBaoFromAdmin(((Player) PLAYERID_OBJECT.get(player.id)),
                                            "Bạn Đã Được Mở Giam Bởi " + player.name);
                                    ServerNotify.gI().sendThongBaoBenDuoi("Người chơi : ("
                                            + ((Player) PLAYERID_OBJECT.get(player.id)).name + ") đã được mãn hạn tù");
                                }
                            }
                        } catch (Exception e) {
                        }
                    ...
                    case ConstNpc.MAKEADMIN:
                        if (select == 0) {
                            ...
                            Service.gI().sendThongBao(player, "Cấp Quyền Admin "
                                    + ((Player) PLAYERID_OBJECT.get(player.id)).name + " thành công");
                        }
                        if (select == 1) {
                            PlayerService.gI().takeAdmin((Player) PLAYERID_OBJECT.get(player.id));
                            Service.gI().sendThongBao(player, "Hủy Quyền Admin "
                                    + ((Player) PLAYERID_OBJECT.get(player.id)).name + " thành công");
                        }
                        break;
                        ...
                        switch (select) {
                            case 0:
                                if (nhandhieu.timedh1 == 0) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (nhandhieu.usedh1 == true ? "Buff" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                            ...
                            case 1:
                                if (nhandhieu.timedh2 == 0) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (nhandhieu.usedh2 == true ? "Buff" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Player đã có DHIEU");
                                }
                            ...
                            case 2:
                                if (nhandhieu.timedh3 == 0) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (nhandhieu.usedh3 == true ? "Buff" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Player đã có DHIEU");
                                }
                            ...
                            case 3:
                                if (nhandhieu.dhtang1 == false) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (nhandhieu.titlett == true ? "Buff" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Player đã có DHIEU");
                                }
                            ...
                            case 5:
                                NpcService.gI().createMenuConMeo(player, ConstNpc.BUFFDHIEU, 12639,
                                        "|7|- •⊹٭DragonBall Kamui٭⊹• -\n|2|TẶNG DANH HIỆU\nPlayer name : "
                                                + nhandhieu.name
                                                + "\n|5|Đại Thần : "
                                                + (nhandhieu.timedh1 > 0 ? Util.msToTime(player.timedh1)
                                                        : nhandhieu.timedh1 <= 0 ? "Chưa Sở Hữu"
                                                                : nhandhieu.usedh1 == true ? "Đang Bật" : "Đang Tắt")
                                                + "\nTrùm Cuối : "
                                                + (nhandhieu.timedh2 > 0 ? Util.msToTime(player.timedh2)
                                                        : nhandhieu.timedh2 <= 0 ? "Chưa Sở Hữu"
                                                                : nhandhieu.usedh2 == true ? "Đang Bật" : "Đang Tắt")
                                                + "\nBất Bại : "
                                                + (nhandhieu.timedh3 > 0 ? Util.msToTime(player.timedh3)
                                                        : nhandhieu.timedh3 <= 0 ? "Chưa Sở Hữu"
                                                                : nhandhieu.usedh3 == true ? "Đang Bật" : "Đang Tắt")
                                                + "\nBuôn Ve Chai : "
                                                + (nhandhieu.dhtang1 == true ? "Đang Bật"
                                                        : nhandhieu.titlett == false ? "Chưa Sở Hữu" : "Đang Tắt")
                                                + "\nFan Cứng : "
                                                + (nhandhieu.dhtang2 == true ? "Đang Bật"
                                                        : nhandhieu.titlett == false ? "Chưa Sở Hữu" : "Đang Tắt"),
                            ...
                            case 0:
                                NpcService.gI().createMenuConMeo(player, ConstNpc.QUY_DOI, 12639,
                                        "|7|DragonBall Kamui" + "\n"
                                                ...
                                                + "\n|7|Tỉ Lệ Quy Đổi Hiện Tại Là : Thỏi Vàng X" + Manager.TLDOITV
                                                + ", Hồng Ngọc X" + Util.format(Manager.TLDOIHN)
                                                + "\n|5|(Giới Hạn Quy Đồi Là 1000 tức không quá 1 triệu Vnđ)"
                                                + "\n" + "Ví Dụ : Nhập 1 = 1.000 Vnđ = " + Manager.TLDOITV
                                                + " Thỏi Vàng "
                                                + "\n" + "Ví Dụ : Nhập 1 = 1.000 Vnđ = "
                                                + Util.format(Manager.TLDOIHN * 1000) + " Hồng Ngọc",
                                        "ĐỔI\nTHỎI VÀNG", "ĐỔI\nHỒNG NGỌC", "Quay lại");
                                break;
                            case 3:
                    ...
                    case ConstNpc.INFO:
                        switch (select) {
                            ...
                            // + "\n|5|Đại Thần (SD +15%, HP,KI +20%) : " + (player.timedh1 > 0 ?
                            // Util.msToTime(player.timedh1) : player.usedh1 == false ? "Có thể nhận qua
                            // event đua top" : "Đang Bật")
                            // + "\nTrùm Cuối (SD +15%, HP,KI +20%) : " + (player.timedh2 > 0 ?
                            // Util.msToTime(player.timedh2) : player.usedh2 == false ? "Có thể nhận qua
                            // event đua top" : "Đang Bật")
                            // + "\nBất Bại (SD +15%, HP,KI +20%) : " + (player.timedh3 > 0 ?
                            // Util.msToTime(player.timedh3) : player.usedh3 == false ? "Có thể nhận qua
                            // event đua top" : "Đang Bật")
                            // + "\n|2|Danh Hiệu Event"
                            // + "\n|5|Buôn Ve Chai (5% SD,HP,KI): " + (player.dhtang1 == true ? "Đang Bật"
                            // : player.titlett == false ? "Có thể nhận được khi tham gia event" : "Đang
                            // Tắt")
                            // + "\nFan Cứng (5% SD,HP,KI) : " + (player.dhtang2 == true ? "Đang Bật" :
                            // player.titlett == false ? "Có thể nhận được khi tham gia event" : "Đang Tắt")
                            // + "\nSức Mạnh (5% SD,HP,KI) : " + (player.dhhanhtinh == true ? "Đang Bật" :
                            // player.titlett == false ? "Có thể nhận được khi đạt 180 Tỉ" : "Đang Tắt")
                            // + "\nPhong Ba (Cập nhật) : " + (player.dhphongba == true ? "Đang Bật" :
                            // player.titlett == false ? "Có thể nhận được khi tham gia event" : "Đang Tắt")
                            // + "\nTuổi Thơ (Cập nhật) : " + (player.dhtuoitho == true ? "Đang Bật" :
                            // player.titlett == false ? "Có thể nhận được khi tham gia event" : "Đang
                            // Tắt"),
                            ...
                            case 1:
                                if (player.pet != null) {
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.IGNORE_MENU, 12639,
                                            "|7|[ Info Player Pet : " + player.pet.getNameThuctinh(player.pet.thuctinh)
                                                    + player.pet.name.substring(1) + " ]\n"
                                                    + "Trạng thái : "
                                                    + (player.pet.status == 1 ? "Bảo Vệ"
                                                            : player.pet.status == 2 ? "Tấn Công"
                                                                    : player.pet.status == 3 ? "Về Nhà"
                                                                            : player.pet.status == 4 ? "Hợp Thể"
                                                                                    : "Đi Theo")
                                                    + "\n|2|Chỉ Số Tổng"
                                                    + "\n|5|HP : " + Util.format(player.pet.nPoint.hp) + "/"
                                                    + Util.format(player.pet.nPoint.hpMax)
                                                    + "\nKI : " + Util.format(player.pet.nPoint.mp) + "/"
                                                    + Util.format(player.pet.nPoint.mpMax)
                                                    ...
                                                    + "\nCRITG : " + Util.format(player.pet.nPoint.critg) + "%",
                                            "Đóng");
                                } else {
                            ...
                            case 0:
                                NpcService.gI().createMenuConMeo(
                                        player, ConstNpc.MENU_PLAYER, 12639, "|7|[ Info Player : " + player.name + " ]"
                                                + "\nTrạng thái tài khoản : "
                                                + (player.getSession().actived == 1 ? " Đã mở thành viên"
                                                        : " Chưa mở thành viên")
                                                + (player.vip == 1 ? " [VIP]"
                                                        : player.vip == 2 ? " [VIP2]"
                                                                : player.vip == 3 ? " [VIP3]"
                                                                        : player.vip == 4 ? " [SVIP]" : "")
                                                + "\n|2|Chỉ Số Tổng"
                                                + "\n|5|HP : " + Util.format(player.nPoint.hp) + "/"
                                                + Util.format(player.nPoint.hpMax)
                                                + "\nKI : " + Util.format(player.nPoint.mp) + "/"
                                                + Util.format(player.nPoint.mpMax)
                        ...
                        switch (select) {
                            case 0:
                                if (player.timedh1 > 0) {
                                    ...
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.usedh1 == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                            ...
                            case 1:
                                if (player.timedh2 > 0) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.usedh2 == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 2:
                                if (player.timedh3 > 0) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.usedh3 == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 3:
                                if (player.titlett == true) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.dhtang1 == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 4:
                                if (player.titlett == true) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.dhtang2 == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 5:
                                if (player.titlett == true) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.dhhanhtinh == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 6:
                                if (player.titlett == true) {
                                    ...
                                    Service.gI().removedh(player);
                                    Service.gI().sendThongBao(player,
                                            "Đã " + (player.dhphongba == true ? "Bật" : "Tắt") + " Danh Hiệu!");
                                    break;
                                } else {
                                    Service.gI().sendThongBao(player, "Ngươi làm gì có danh hiệu");
                                }
                            ...
                            case 8:
                                NpcService.gI().createMenuConMeo(player, ConstNpc.CHECK_DH, 12639,
                                        "|7|- •⊹٭DragonBall Kamui٭⊹• -\nTITLE SERVICE\n"
                                                + "|2|Danh Hiệu Top Event"
                                                + "\n|5|Đại Thần (SD +15%, HP,KI +20%) : "
                                                + (player.timedh1 > 0 ? Util.msToTime(player.timedh1)
                                                        : player.usedh1 == false ? "Có thể nhận qua event đua top"
                                                                : "Đang Bật")
                                                + "\nTrùm Cuối (SD +15%, HP,KI +20%) : "
                                                + (player.timedh2 > 0 ? Util.msToTime(player.timedh2)
                                                        : player.usedh2 == false ? "Có thể nhận qua event đua top"
                                                                : "Đang Bật")
                                                + "\nBất Bại (SD +15%, HP,KI +20%) : "
                                                + (player.timedh3 > 0 ? Util.msToTime(player.timedh3)
                                                        : player.usedh3 == false ? "Có thể nhận qua event đua top"
                                                                : "Đang Bật")
                                                + "\n|2|Danh Hiệu Event"
                                                + "\n|5|Buôn Ve Chai (5% SD,HP,KI): "
                                                + (player.dhtang1 == true ? "Đang Bật"
                                                        : player.titlett == false
                                                                ? "Có thể nhận được khi tham gia event"
                                                                : "Đang Tắt")
                                                + "\nFan Cứng (5% SD,HP,KI) : "
                                                + (player.dhtang2 == true ? "Đang Bật"
                                                        : player.titlett == false
                                                                ? "Có thể nhận được khi tham gia event"
                                                                : "Đang Tắt")
                                                + "\nSức Mạnh (5% SD,HP,KI) : "
                                                + (player.dhhanhtinh == true ? "Đang Bật"
                                                        : player.titlett == false ? "Có thể nhận được khi đạt 180 Tỉ"
                                                                : "Đang Tắt")
                                                + "\nPhong Ba (Cập nhật) : "
                                                + (player.dhphongba == true ? "Đang Bật"
                                                        : player.titlett == false
                                                                ? "Có thể nhận được khi tham gia event"
                                                                : "Đang Tắt")
                                                + "\nTuổi Thơ (Cập nhật) : "
                                                + (player.dhtuoitho == true ? "Đang Bật"
                                                        : player.titlett == false
                                                                ? "Có thể nhận được khi tham gia event"
                                                                : "Đang Tắt"),
                                        ("Đại Thần\n" + (player.usedh1 == true ? "'ON'" : "'OFF'")),
                    ...
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.IGNORE_MENU, "|5|Có 2 nhà cái Tài và Xĩu,
                    // bạn chỉ được chọn 1 nhà để tham gia"
                    // + "\n\n|6|Sau khi kết thúc thời gian đặt cược. Hệ thống sẽ tung xí ngầu để
                    // biết kết quả Tài Xỉu"
                    // + "\n\nNếu Tổng số 3 con xí ngầu <=10 : XỈU\nNếu Tổng số 3 con xí ngầu >10 :
                    // TÀI\nNếu 3 Xí ngầu cùng 1 số : TAM HOA (Nhà cái lụm hết)"
                    // + "\n\n|7|Lưu ý: Số Hồng ngọc nhận được sẽ bị nhà cái lụm đi 20%. Trong quá
                    // trình diễn ra khi đặt cược nếu thoát game sẽ bị MẤT TIỀN ĐẶT CƯỢC", "Ok");
                    // break;
                    // case 1:
                    // String time = ((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) + " giây";
                    ...
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    // : " + TaiXiu_Old.gI().z + " Tổng : " + (TaiXiu_Old.gI().x + TaiXiu_Old.gI().y
                    // + TaiXiu_Old.gI().z)
                    // + "\n\n|6|Tổng nhà TÀI: " + Util.format(TaiXiu_Old.gI().goldTai) + " Hồng
                    // ngọc"
                    // + "\nTổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
                    ...
                    // 1000) + " giây";
                    // if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) > 0
                    // && player.goldTai == 0 && player.goldXiu == 0 && TaiXiu_Old.gI().baotri ==
                    // false) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    ...
                    // }
                    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) > 0 && player.goldTai > 0 && TaiXiu_Old.gI().baotri == false) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    ...
                    // + "\nTổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
                    // ngọc\n\n|5|Thời gian còn lại: " + time + "\n\n|7|Bạn đã cược Tài : " +
                    // Util.format(player.goldTai) + " Hồng ngọc", "Cập nhập", "Đóng");
                    // break;
                    // }
                    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) > 0 && player.goldXiu > 0 && TaiXiu_Old.gI().baotri == false) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    ...
                    // + "\nTổng nhà XỈU: " + Util.format(TaiXiu_Old.gI().goldXiu) + " Hồng
                    // ngọc\n\n|5|Thời gian còn lại: " + time + "\n\n|7|Bạn đã cược Xỉu : " +
                    // Util.format(player.goldXiu) + " Hồng ngọc", "Cập nhập", "Đóng");
                    // break;
                    // }
                    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) > 0 && player.goldTai > 0 && TaiXiu_Old.gI().baotri == true) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    ...
                    // }
                    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) > 0 && player.goldXiu > 0 && TaiXiu_Old.gI().baotri == true) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    ...
                    // }
                    // } else if (((TaiXiu_Old.gI().lastTimeEnd - System.currentTimeMillis()) /
                    // 1000) > 0 && player.goldXiu == 0 && player.goldTai == 0 &&
                    // TaiXiu_Old.gI().baotri == true) {
                    // switch (select) {
                    // case 0:
                    // createOtherMenu(player, ConstNpc.TAIXIU, "\n|7|- NHÀ CÁI TÀI XỈU Kamui
                    // -\n|3|Kết quả kì trước: " + TaiXiu_Old.gI().x + " : " + TaiXiu_Old.gI().y + "
                    ...
                    case ConstNpc.MUA_DE:
                        switch (select) {
                            /*
                             * case 0:
                             * this.createOtherMenu(player, ConstNpc.CONFIRM_MUA_DE_BERUS,
                             * "Bạn có chắc muốn mua đệ Berus không?", "Đồng ý", "Không");
                             * break;
                             */
                            case 0:
                                this.createOtherMenu(player, ConstNpc.CONFIRM_MUA_DE_KEFLA,
                                        "Bạn có chắc muốn mua đệ Cumber SSJ2(30%) không?", "Đồng ý", "Không");
                                break;
                            case 1:
                                // Service.gI().sendThongBao(player, "Hiện tại chưa mở bán");
                                this.createOtherMenu(player, ConstNpc.CONFIRM_MUA_DE_CUMBER,
                                        "Bạn có chắc muốn mua đệ Goku SSJ4 (50%) không?\nMua đệ này kèm theo cải trang ssj4 hoặc cải trang có option Gogeta\nkhi hợp thể tăng 70%",
                                        "Đồng ý", "Không");
                                break;
                            case 2:
                                // Service.gI().sendThongBao(player, "Hiện tại chưa mở bán");
                                this.createOtherMenu(player, ConstNpc.CONFIRM_MUA_DE_GOKU,
                                        "Bạn có chắc muốn mua đệ Goku SSJ4 (50%) không?\nMua đệ này kèm theo cải trang ssj4 hoặc cải trang có option Gogeta\nkhi hợp thể tăng 70%",
                                        "Đồng ý", "Không");
                                break;
                        }
                            ...
                            case 0:
                                if (player.getSession().actived == 0) {
                                    ...
                                    if (player.pet != null) {
                                        ...
                                        Service.gI().sendThongBao(player,
                                                "Hiện bạn chưa có đệ tử, hãy săn đệ và quay lại sau");
                                    }
                                }
                                break;
                            case 1:
                        ...
                        switch (select) {
                            case 0:
                                if (player.getSession().vnd < 150000) {
                                    ...
                                    if (player.pet != null
                                            && player.pet.inventory.itemsBody.get(0).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(1).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(2).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(3).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(4).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(5).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(6).isNotNullItem() == false
                                            && player.pet.inventory.itemsBody.get(7).isNotNullItem() == false) {
                                        try {
                                            int sum = player.getSession().vnd - 150000;
                                            GirlkunDB.executeUpdate(
                                                    "update account set active = ?, vnd = ? where id = ?", 1, sum,
                                ...
                                if (player.getSession().vnd < 20000) {
                                    ...
                                    if (player.pet.playerSkill.skills.get(2).skillId != -1
                                        ...
                                        try {
                                            ...
                                            GirlkunDB.executeUpdate("update account set vnd = ? where id = ?", sum,
                                                    player.account_id);
                                            player.getSession().vnd = sum;
                                        } catch (Exception e) {
                                            this.npcChat(player, "Đã có lỗi xảy ra!" + e);
                    ...
                    case ConstNpc.CHISO_NHANH:
                        switch (select) {
                            case 0:
                                ...
                                Service.gI().sendThongBao(player,
                                        "|1|Đã " + (player.autoHP == true ? "Bật" : "Tắt") + " Auto cộng chỉ số HP");
                                try {
                                    while (player.autoHP == true) {
                                        if (player.nPoint != null
                                                && player.nPoint.hpg + 20000 <= player.nPoint.getHpMpLimit()) {
                                            player.nPoint.increasePoint((byte) 0, (short) 1000);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng HP!\n|2|" + player.nPoint.hpg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.hpg + 2000 <= player.nPoint.getHpMpLimit()) {
                                            player.nPoint.increasePoint((byte) 0, (short) 100);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng HP!\n|2|" + player.nPoint.hpg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.hpg + 200 <= player.nPoint.getHpMpLimit()) {
                                            player.nPoint.increasePoint((byte) 0, (short) 10);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng HP!\n|2|" + player.nPoint.hpg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.hpg + 20 <= player.nPoint.getHpMpLimit()) {
                            ...
                            case 1:
                                ...
                                Service.gI().sendThongBao(player,
                                        "|1|Đã " + (player.autoKI == true ? "Bật" : "Tắt") + " Auto cộng chỉ số KI");
                                if (player.autoKI == true) {
                                    try {
                                        while (player.autoKI == true) {
                                            if (player.nPoint != null
                                                    && player.nPoint.mpg + 20000 <= player.nPoint.getHpMpLimit()) {
                                                player.nPoint.increasePoint((byte) 1, (short) 1000);
                                                Service.gI().sendThongBao(player,
                                                        "|8|Auto cộng KI!\n|2|" + player.nPoint.mpg);
                                            } else if (player.nPoint != null
                                                    && player.nPoint.mpg + 2000 <= player.nPoint.getHpMpLimit()) {
                                                player.nPoint.increasePoint((byte) 1, (short) 100);
                                                Service.gI().sendThongBao(player,
                                                        "|8|Auto cộng KI!\n|2|" + player.nPoint.mpg);
                                            } else if (player.nPoint != null
                                                    && player.nPoint.mpg + 200 <= player.nPoint.getHpMpLimit()) {
                                                player.nPoint.increasePoint((byte) 1, (short) 10);
                                                Service.gI().sendThongBao(player,
                                                        "|8|Auto cộng KI!\n|2|" + player.nPoint.mpg);
                                            } else if (player.nPoint != null
                                                    && player.nPoint.mpg + 20 <= player.nPoint.getHpMpLimit()) {
                            ...
                            case 2:
                                ...
                                Service.gI().sendThongBao(player,
                                        "|1|Đã " + (player.autoSD == true ? "Bật" : "Tắt") + " Auto cộng chỉ số KI");
                                try {
                                    while (player.autoSD == true) {
                                        if (player.nPoint != null
                                                && player.nPoint.dameg + 1000 <= player.nPoint.getDameLimit()) {
                                            player.nPoint.increasePoint((byte) 2, (short) 1000);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng SĐ!\n|2|" + player.nPoint.dameg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.dameg + 100 <= player.nPoint.getDameLimit()) {
                                            player.nPoint.increasePoint((byte) 2, (short) 100);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng SĐ!\n|2|" + player.nPoint.dameg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.dameg + 10 <= player.nPoint.getDameLimit()) {
                                            player.nPoint.increasePoint((byte) 2, (short) 10);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng SĐ!\n|2|" + player.nPoint.dameg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.dameg + 1 <= player.nPoint.getDameLimit()) {
                            ...
                            case 3:
                                ...
                                Service.gI().sendThongBao(player,
                                        "|1|Đã " + (player.autoGiap == true ? "Bật" : "Tắt") + " Auto cộng chỉ số DEF");
                                try {
                                    while (player.autoGiap == true) {
                                        if (player.nPoint != null
                                                && player.nPoint.defg + 100 <= player.nPoint.getDefLimit()) {
                                            player.nPoint.increasePoint((byte) 3, (short) 100);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng DEF!\n|2|" + player.nPoint.defg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.defg + 10 <= player.nPoint.getDefLimit()) {
                                            player.nPoint.increasePoint((byte) 3, (short) 10);
                                            Service.gI().sendThongBao(player,
                                                    "|8|Auto cộng DEF!\n|2|" + player.nPoint.defg);
                                        } else if (player.nPoint != null
                                                && player.nPoint.defg + 1 <= player.nPoint.getDefLimit()) {
                    ...
                    case ConstNpc.MENU_ADMIN:
                        switch (select) {
                            ...
                            case 6:
                                // if (player.isAdmin2()) {
                                // try {
                                // GirlkunResultSet rs = GirlkunDB.executeQuery("SELECT * FROM a_server_sukien
                                // WHERE id_event > 0");
                            ...
                            case 7:
                                ...
                                // Service.gI().sendThongBao(player, "Đã " + (TaiXiu_Old.gI().baotri == true ?
                                // "Tắt" : "Bật") + " Tài Xỉu!");
                                break;
                            case 8:
                                // ChonAiDay.gI().baotri = !ChonAiDay.gI().baotri;
                                // Service.gI().sendThongBao(player, "Đã " + (ChonAiDay.gI().baotri == true ?
                                // "Tắt" : "Bật") + " Lý Tiểu Nương!");
                                break;
                            case 9:
                             ...
                             * }
                             * Service.gI().sendThongBaoFromAdmin(player,
                             * "|5|YÊU CẦU XÓA ALL LIST BOSS TRƯỚC!");
                             * break;
                             * } catch (InterruptedException e) {
                             * Service.gI().sendThongBao(player, "Reset Lỗi");
                             * }
                        ...
                        switch (select) {
                            case 0:
                                if (player.isAdmin() || (player.isAdmin2())) {
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.MOD, 12639,
                                            "|7|[ - Menu Key - ]" + "\n"
                                                    + "|2|Số Người Chơi Đang Online : "
                            ...
                            case 1:
                                if (player.isAdmin()) {
                                    ...
                                    if (player.isAdmin2()) {
                                        NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, 12639,
                                                "|7|[ - Menu Quản Trị - ]" + "\n"
                                                        + "|2|Số Người Chơi Đang Online : "
                                                        + Client.gI().getPlayers().size()
                                                        + "\nServer Run Time : " + ServerManager.timeStart
                                                ...
                                                "SỰ KIỆN\n" + (ConstEvent.EVENT == 1 ? "(MÙA HÈ)"
                                                        : ConstEvent.EVENT == 2 ? "(H.VƯƠNG)"
                                                                : ConstEvent.EVENT == 3 ? "(TRUNG THU)"
                                                                        : ConstEvent.EVENT == 4 ? "(HLWEEN)"
                                                                                : ConstEvent.EVENT == 5 ? "(20/11)"
                                                                                        : ConstEvent.EVENT == 6
                                                                                                ? "(NOEL)"
                                                                                                : ConstEvent.EVENT == 7
                                                                                                        ? "(TẾT)"
                                                                                                        : ConstEvent.EVENT == 8
                                                                                                                ? "(8/3)"
                                                                                                                : "(ĐÓNG)"),
                                                // "TÀI XỈU\n" + (TaiXiu_Old.gI().baotri == true ? "(OFF)" : "(ON)"),
                                                // "CHẴN LẺ\n" + (ChonAiDay.gI().baotri == true ? "(OFF)" : "(ON)"),
                                                "EXP PET\nX" + Manager.TNPET + "\n(" + Util.format(Manager.TNPET * 20)
                                                        + "p/s)",
                                                "TỈ LỆ NẠP\nTV : X" + Manager.TLDOITV + "\nHN : X"
                                                        + Util.format(Manager.TLDOIHN));
                                    }
                                }
                                break;
                            case 2:
                    ...
                    // "|7|[ BANED PLAYER ]\n" + "Bạn có chắc chắn muốn ban : " + p.name, selects,
                    // p);
                    // break;
                    // case 4:
                    // if (p.isAdmin2()) {
                    ...
                    // break;
                    // }
                    // Service.gI().sendThongBao(player, "Kick người chơi " + p.name + " thành
                    // công");
                                ...
                                case 5:
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.BUFFDHIEU, 12639,
                                            "|7|- •⊹٭DragonBall Kamui٭⊹• -\n|2|TẶNG DANH HIỆU\nPlayer name : "
                                                    + plql.name
                                                    + "\n|5|Đại Thần : "
                                                    + (plql.timedh1 > 0 ? Util.msToTime(plql.timedh1)
                                                            : plql.timedh1 <= 0 ? "Chưa Sở Hữu"
                                                                    : plql.usedh1 == true ? "Đang Bật" : "Đang Tắt")
                                                    + "\nTrùm Cuối : "
                                                    + (plql.timedh2 > 0 ? Util.msToTime(plql.timedh2)
                                                            : plql.timedh2 <= 0 ? "Chưa Sở Hữu"
                                                                    : plql.usedh2 == true ? "Đang Bật" : "Đang Tắt")
                                                    + "\nBất Bại : "
                                                    + (plql.timedh3 > 0 ? Util.msToTime(plql.timedh3)
                                                            : plql.timedh3 <= 0 ? "Chưa Sở Hữu"
                                                                    : plql.usedh3 == true ? "Đang Bật" : "Đang Tắt")
                                                    + "\nBuôn Ve Chai : "
                                                    + (plql.dhtang1 == true ? "Đang Bật"
                                                            : plql.titlett == false ? "Chưa Sở Hữu" : "Đang Tắt")
                                                    + "\nFan Cứng : "
                                                    + (plql.dhtang2 == true ? "Đang Bật"
                                                            : plql.titlett == false ? "Chưa Sở Hữu" : "Đang Tắt"),
                                            "Đại Thần", "Trùm Cuối", "Bất Bại", "Ve Chai", "Fan Cứng", "Làm Mới");
                                    break;
                                case 6:
